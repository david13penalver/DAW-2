...
extension ExerciseInteractorImpl {
    ...
    func updateExercise(exercise: ExerciseModel) async throws {
        ...
        let dto = ExerciseDTO(exerciseId: exercise.id ?? 0, name: exercise.name, imageURL: exercise.imageURL ?? "", videoURL: exercise.videoURL, isGlobal: exercise.isGlobal, description: exercise.description, exerciseType: exerciseTypeDTO, exerciseSubtype: exerciseSubtypeDTO)
        print(dto)
        try await api.updateExercise(dto: dto)
    }
