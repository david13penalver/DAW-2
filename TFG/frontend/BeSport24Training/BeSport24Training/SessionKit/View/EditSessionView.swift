    ...
    var body: some View {
        ...
        NavigationStack {
            VStack {
                ...
                Button("Edit") {
                    Task {
                        await vm.updateSession(name: name, description: description)
                        await vm.findSessionById(id: vm.session!.id)
                        isPresented = false
                    }
                }
...
#Preview {
    EditSessionView(isPresented: .constant(true), vm: SessionViewModel())
}
