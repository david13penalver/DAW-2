    ...
    var body: some View {
        ...
        NavigationStack {
            VStack {
                ...
                Button("Edit") {
                    Task {
                        await vm.updateTraining(name: name, description: description)
                        await vm.findTrainingById(id: vm.training!.id)
                        isPresented = false
                    }
                }
...
#Preview {
    EditTrainingView(isPresented: .constant(true), vm: TrainingViewModel())
}
