...
public class AuthorController {
    ...
    private final BookGetAllBooksByAuthor bookGetAllBooksByAuthor;

    @GetMapping("/{id}/books")
    public ResponseEntity<PaginatedResponse<BookCollection>> getAllBooksByAuthorId(@PathVariable Long id,
                                                                @RequestParam(defaultValue = "1") int page,
                                                                @RequestParam(required = false) Integer size) {
        int pageSize = (size != null) ? size : Integer.parseInt(defaultPageSize);
        String baseUrl = PropertiesConfig.getSetting("app.base.url") + URL;
        ListWithCount<Book> bookList = bookGetAllBooksByAuthor.execute(id,page - 1, pageSize);
